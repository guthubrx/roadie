...
- [x] T070 [P3] Ajouter un test E2E pour le scenario hot-plug : connecter/déconnecter un display pendant que le daemon tourne, vérifier que `activeStageByDesktop` reste cohérent. — **[ABANDONNÉE — couverte par daily-driving]** : le user driver multi-display avec branchements/débranchements occasionnels du LG. Aucune incohérence de `activeStageByDesktop` observée. Test E2E synthétique reporté à un éventuel SPEC dédié hot-plug si une régression apparaît.
- [x] T071 [P3] Documenter la nouvelle sémantique de `switchTo` dans `CLAUDE.md` ou un `README` interne pour les futurs contributeurs. — **[ABANDONNÉE — documentée in-code]** : la sémantique `switchTo(stageID:scope:)` est commentée directement dans `Sources/RoadieStagePlugin/StageManager.swift` (commentaires SPEC-022 + Decisions notables consignées dans `implementation.md`). CLAUDE.md vise les conventions transverses, pas les détails par module.

## Tâches imprévues ajoutées post-merge (Loi de Conservation)

Polish/correctifs déclenchés par le daily-driving de SPEC-022 après le merge initial `09ab11d`. Tous au scope SPEC-022 (multi-display isolation), prolongement direct des US1/US2/US3.

- [x] T100 [Hors-plan post-merge] `feat(SPEC-022): T065 warn boot active.toml deprecated + tests + tasks.md sync` (commit `2576cd0`) — log de warning explicite au boot quand l'ancien `~/.config/roadies/stages/active.toml` global existe encore.
- [x] T101 [Hors-plan post-merge] `fix(SPEC-022): switchTo legacy scope-aware + ordre stable stage.list` (commit `74c0b8d`) — l'ancien `switchTo(stageID:)` non scopé devient lui aussi scope-aware en utilisant currentDesktopKey. `stage.list` retourne désormais un ordre stable par scope (alphabétique sur StageID).
- [x] T102 [Hors-plan post-merge] `fix(SPEC-022): axDidMoveWindow migre stagesV2 ownership sur drag cross-display` (commit `23ca417`) — quand l'utilisateur drague une fenêtre d'un display à l'autre, l'ownership stagesV2 est migré vers le scope cible (display, desktop, active stage). Sans ça, le rail panel source affichait toujours la vignette alors que la fenêtre était physiquement sur l'autre display.
- [x] T103 [Hors-plan post-merge] `fix(SPEC-022): auto-assign per-display physique au registerWindow + boot` (commit `8789848`) — au registerWindow et au boot, chaque wid est auto-assignée au scope dont l'écran physique contient le centre de sa frame. Évite que toutes les wids démarrent assignées au built-in display.
- [x] T104 [Hors-plan post-merge] `fix(SPEC-022): LayoutEngine display-aware (rebuild trees per display)` (commit `270c73c`) — `applyAll` itère par display avec `activeStageByDisplay[display.id]` au lieu d'un scalaire global. Résout la limitation connue documentée dans REX implementation.md "LayoutEngine.workspace.activeStageID reste global".
- [x] T105 [Hors-plan post-merge] `fix(SPEC-022): rail refreshThumbnails iter par display (multi-display fix)` (commit `38208f8`) — `refreshThumbnails` itère par display pour ne capturer que les wids physiquement présentes sur ce display, plus une seule passe globale.
- [x] T106 [Hors-plan post-merge] `refactor(SPEC-022): activeStageID global → activeStageByDisplay (per-display)` (commit `0976992`) — `Workspace.activeStageID` (scalaire) déprécié au profit de `activeStageByDisplay: [CGDirectDisplayID: StageID]`. Permet à chaque display d'afficher une stage différente simultanément côté tiling. Computed property maintenue pour compat.
- [x] T107 [Hors-plan post-merge] `refactor(SPEC-022): finalisation audit V1 dict — sites scope-blind restants` (commit `35d13c4`) — purge des derniers sites internes encore scope-blind (lecture/écriture du V1 `stages: [StageID: Stage]` au lieu du V2 `stagesV2: [StageScope: Stage]`).
- [x] T108 [Hors-plan post-merge] `fix(SPEC-022): assign + auto-switch scope = display physique de la wid` (commit `683c023`) — `stage.assign` sans `--display` utilise désormais le display physique de la wid (frame center → display) au lieu du curseur, pour cohérence avec l'intent utilisateur. `assign_follows_focus` utilise `assignedScope` au lieu de `currentStageID` global.
- [x] T109 [Hors-plan post-merge] `fix(SPEC-022): switchTo legacy + setActiveStage hook display-aware` (commit `c37c5b1`) — `LayoutHooks.setActiveStage` étendu de `(StageID?)` à `(StageID?, String?)` (avec displayUUID), tous les call-sites passent `currentDesktopKey?.displayUUID`. Fin de la pollution cross-display.
- [x] T110 [Hors-plan post-merge] `fix(SPEC-022): respect du modèle « étiquette logique pilotée par l'user »` (commit `6284ce6`) — désactivation du re-étiquetage automatique SkyLight basé sur la position physique observée (Mission Control Space tracking). Le modèle utilisateur est : étiquette logique d'une fenêtre pilotée uniquement par les actions explicites (drag manuel, raccourci `window display N`, drag rail), jamais par observation passive de la position.
- [x] T111 [Hors-plan post-merge] `feat(SPEC-022): GC d'intégrité périodique + filtres anti-frame-degenerate` (commit `ee549f9`) — `WindowDesktopReconciler` refactor en GC d'intégrité (3 checks/tick : frames degenerate, offscreen-with-active-scope, tree-leaf-wrong-display). `WindowRegistry.updateFrame` rejette les frames degenerate (width OR height < 100). Évite les corruptions transitoires d'AX events sur drawer/popup.
