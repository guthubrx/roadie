...
- [x] T021 [US2] Étendre `LayoutEngine` : méthode `func moveWindow(_ wid:, fromDisplay src:, toDisplay dst:)` qui retire le wid de `rootsByDisplay[src]` et insère dans `rootsByDisplay[dst]` (R-005).
- [x] T022 [US2] Câbler dans `Sources/roadied/CommandRouter.swift` : nouveau handler `case "window.display"` qui : (1) valide selector range 1..count, (2) résout `from` via current frame, (3) calcule new frame centrée dans `dst.visibleFrame` avec clamp si dépasse, (4) `AXReader.setBounds`, (5) `LayoutEngine.moveWindow`, (6) update `WindowEntry.displayUUID` via DesktopRegistry, 
(7) `applyLayout(displayID: src)` + `applyLayout(displayID: dst)`. Conforme FR-008, contracts/cli-window-display.md.
- [x] T023 [US2] Étendre `Sources/roadie/main.swift` : sous-commande `window display <selector>`. Selectors `1..N`, `prev`, `next`, `main` (FR-014). Pipe vers daemon socket.
- [x] T024 [P] [US2] Tests intégration : MockDisplayRegistry 2 écrans, fenêtre sur écran 1, appel `window.display 2`, vérifier WindowEntry.displayUUID == écran 2 + frame mise à jour.
- [x] T025 [P] [US2] Test selector invalide : `window.display 5` avec 2 écrans → erreur `unknown_display`.
...
- [x] T026 [US3] Étendre `DisplayRegistry.handleScreenChange()` (interne, déclenchée par observer T009) : compute diff `oldDisplays` vs `newDisplays` (R-006, FR-015).
- [x] T027 [US3] Pour chaque écran retiré : itérer sur les fenêtres rattachées (via `LayoutEngine.rootsByDisplay[oldID]`), pour chaque wid : ajuster sa frame pour le mettre dans le visibleFrame du primary (clamp + shift), `AXReader.setBounds`, `LayoutEngine.moveWindow(from:oldID, to:primaryID)`, update WindowEntry.displayUUID = primary.uuid.
- [x] T028 [US3] Pour chaque écran ajouté : initialiser `rootsByDisplay[newID] = TilingContainer()` vide.
- [x] T029 [US3] Émettre `display_configuration_changed` event (FR-023).
- [x] T030 [P] [US3] Tests `Tests/RoadieTilerTests/DisplayRegistryRecoveryTests.swift` : MockProvider initial 2 displays + 2 fenêtres sur display 2 ; remove display 2 ; vérifier que fenêtres sont migrées et leurs frames sont dans visibleFrame du primary.
- [x] T031 [P] [US3] Test perf : cycle connect/disconnect 10 fois : 0 fenêtre fantôme (SC-006), perf < 500 ms par cycle (SC-003).
...
- [x] T032 [US4] Câbler `Sources/roadied/CommandRouter.swift` : handler `case "display.list"` qui appelle `daemon.displayRegistry.displays`, formate avec `windows: count` par display (depuis `LayoutEngine.rootsByDisplay[displayID].leafCount`). Conforme contracts/cli-display.md (FR-011).
- [x] T033 [US4] Câbler handler `case "display.current"` : retourne le display contenant la fenêtre frontmost (FR-012).
- [x] T034 [US4] Câbler handler `case "display.focus"` : selector → display, focus la fenêtre frontmost de l'écran ou la première leaf tilée (FR-013).
- [x] T035 [US4] Étendre `Sources/roadie/main.swift` : sous-commande `display list/current/focus`.
- [x] T036 [P] [US4] Tests CLI : socket round-trip avec MockDisplayRegistry sur configurations 1, 2, 3 écrans (SC-005), vérifier output.
...
- [x] T037 [US5] Étendre `Sources/RoadieCore/Config.swift` : ajouter struct `DisplayRule` (matchIndex, matchUUID, matchName, defaultStrategy, gapsOuter, gapsInner) ; champ `displays: [DisplayRule]` sur `Config`. Parser section `[[displays]]` (R-008, FR-018).
- [x] T038 [US5] Au boot et après chaque `refresh()` du DisplayRegistry, appliquer les rules : pour chaque Display, chercher matchIndex/matchUUID/matchName puis copier les overrides dans `display.tilerStrategy/gapsOuter/gapsInner` (FR-019).
- [x] T039 [P] [US5] Tests `Tests/RoadieCoreTests/ConfigDisplaysTests.swift` : parser TOML 2 rules, vérifier match par index/uuid/name.
- [x] T040 [P] [US5] Test E2E : 2 mock displays, config rule pour display 2 = master_stack, vérifier `display.list` retourne stratégies différentes.
...
- [x] T041 [US6] Étendre `DisplayRegistry.setActive(id:)` : si l'id change, émettre `display_changed` event via le bus daemon (R-009, FR-022).
- [x] T042 [US6] Câbler dans `Sources/roadied/main.swift` : à chaque `axDidChangeFocusedWindow`, recalculer le display actif (via centre de la frame) et appeler `displayRegistry.setActive(id:)`.
- [x] T043 [US6] Émettre `display_configuration_changed` à chaque `refresh()` quand la liste a changé (FR-023, déjà T029).
- [x] T044 [P] [US6] Tests `Tests/RoadieDesktopsTests/DisplayEventsTests.swift` : subscriber + change activeID → event reçu < 50 ms.
...
- [x] T045 [US7] Re-runner toute la suite SPEC-011 (`swift test --filter RoadieDesktops --filter RoadieStagePlugin`) après chaque sprint US1..US4. Tous tests doivent rester verts (FR-024, SC-004).
- [x] T046 [US7] Tester `roadie display list` sur mono-écran : retourne 1 ligne identique (FR-011).
- [x] T047 [US7] Tester `roadie window display 2` sur mono-écran : erreur `unknown_display` (FR-010).

---

## Phase 10 : Polish & Cross-Cutting

- [x] T048 [P] Vérifier LOC `Sources/RoadieCore/Display*.swift` + nouveaux fichiers SPEC-012 < 800 LOC effectives (plafond plan.md)
- [x] T049 [P] Vérifier que la suite tests SPEC-011+SPEC-012 passe complètement : ≥ 200 tests verts cible
- [x] T050 [P] Re-run linter `bash Tests/StaticChecks/no-cgs.sh` étendu (SC-007)
- [x] T051 [P] Mettre à jour `README.md` avec la section multi-display (≤ 200 mots, SC-010)
- [x] T052 [P] Mettre à jour `CHANGELOG.md` : entry SPEC-012
- [ ] T053 Mesurer perf SC-002 manuellement : 10 mesures de `time roadie window display 2` < 200 ms p95
- [x] T054 Backward-compat : tester sur un state.toml SPEC-011 existant (sans `display_uuid`), vérifier load propre + auto-fill du champ au prochain save (FR-021, R-012)
